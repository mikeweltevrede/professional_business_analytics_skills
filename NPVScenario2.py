# -*- coding: utf-8 -*-
"""
Created on Wed Apr 29 15:17:17 2020
Scenario 2
Each market produces at least 10% of the Capacity 
"""
...
def NPV_SAA2(Data, w, h):
    
    Scenarios = len(Data)
    Products = len(Data[0]['ProductSize'])

    Time = len(Data[0]['ProductPrice'].columns) - 2
    Notebook = Data[0]['ProductSize'].loc[Data[0]['ProductSize']['Market']=='Notebook'].index.values
    Monitor = Data[0]['ProductSize'].loc[Data[0]['ProductSize']['Market']=='Monitor'].index.values
    Television = Data[0]['ProductSize'].loc[Data[0]['ProductSize']['Market']=='Television'].index.values
    ...
    for t in range(2,Time): # Start at t = 2, yield for t = 0 and 1 is equal to 0
        m.addConstr(quicksum(x[p,t] for p in Notebook) >= 0.1*Data[0]['MaxCapacity']*12 ) #NOTEBOOKS
        m.addConstr(quicksum(x[p,t] for p in Monitor) >= 0.1*Data[0]['MaxCapacity']*12 ) #MONITOR
        m.addConstr(quicksum(x[p,t] for p in Television) >= 0.1*Data[0]['MaxCapacity']*12 ) #TV
