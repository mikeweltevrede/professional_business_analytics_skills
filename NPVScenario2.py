# -*- coding: utf-8 -*-
"""
Created on Fri Apr 24 09:56:49 2020

PBAS Scenario 2
"""

def NPV_SAA_Scenario2(Data,w,h):
    ...
    for t in range(2,Time):
        m.addConstr(quicksum(x[p,t] for p in range(2)) >= 0.1*Data[0]['MaxCapacity']*12 ) #NOTEBOOKS
        m.addConstr(quicksum(x[p,t] for p in range(2,5)) >= 0.1*Data[0]['MaxCapacity']*12 ) #MONITOR
        m.addConstr(quicksum(x[p,t] for p in range(5,Products)) >= 0.1*Data[0]['MaxCapacity']*12 ) #TV
