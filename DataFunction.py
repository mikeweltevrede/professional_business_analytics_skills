...
def generateData(path):
    ...
    for j in range(5, 12):
        ProductInches.values[j, 0] = ProductSize.values[j, 0] + \
            np.random.choice(outcome, 1, probability)

    for i in range(9, 12):
        ProductInches.values[i, 1] = math.atan(
            ProductFormat.values[0, 1]/ProductFormat.values[1, 1])
    for i in range(6, 9):
        ProductInches.values[i, 1] = math.atan(
            ProductFormat.values[0, 2]/ProductFormat.values[1, 2])
    ...
    CostSubstrate2 = pd.DataFrame(
        np.zeros((1, len(CostSubstrate.columns[3:18]))), columns=CostSubstrate.columns[3:18])
    for i in range(2, len(CostSubstrate.columns[3:18])):
        CostSubstrate2.values[0, i] = np.random.choice(
            outcome3, 1, probability) * CostSubstrate.values[0, i+3]

    # Investment Cost over time including the uncertainty
    # Plus or minus 10% so use same outcome as for the cost substrate
    CostInvestment2 = pd.DataFrame(
        np.zeros((1, len(CostInvestment.columns[3:18]))), columns=CostInvestment.columns[3:18])
    for i in range(3):
        CostInvestment2.values[0, i] = np.random.choice(
            outcome3, 1, probability) * CostInvestment.values[0, i+3]*1000000

    # Yield per market over time including the uncertainty
    outcome4 = [0.85, 1, 1.02]  # 15% down or 2% up
    Yield2 = pd.DataFrame(np.zeros((len(ProductInches), len(
        Yield.columns[3:18]))), index=ProductInches.index, columns=Yield.columns[3:18])
    ...
    for j in range(2, len(Yield.columns[3:18])):
        Yield2.values[5:len(ProductInches), j] = np.random.choice(
            outcome4, 1, probability) * Yield.values[2, j+3]
