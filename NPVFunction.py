...
def NPV_SAA(Data, w, h):
    ...
    for s in range(Scenarios):
        for t in range(Time):
            m.addConstr(Sales[s, t] == quicksum(Data[s]['ProductPrice'].values[p, t]*Data[s]
                                                ['Yield'].values[p, t]*ApS[s][p]*x[p, t]
                                                for p in range(Products)), name='Sales constraint')

    for s in range(Scenarios):
        for t in range(Time):
            m.addConstr(CostofSales[s, t] == quicksum(
                Data[s]['SubstrateCost'].values[0, t]*(w*h)*x[p, t] for p in range(Products)) +
                Data[s]['Depreciation'].values[0, t], name='Cost of Sales constraint')
    ...
    for s in range(Scenarios):  # DWC moet er nog bij.
        for t in range(Time):
            NCF[s, t] = (NI[s, t] + Data[s]['Depreciation'].values[0, t] - DWC[s, t] -
                         Data[s]['InvestmentCost'].values[0, t])
